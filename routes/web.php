...
Route::name(config('admin.route.name'))->prefix(config('admin.route.prefix'))->middleware(['auth', 'admin'])->group(function () {
    ...
    Route::resource('logistics', \App\Http\Controllers\Admin\LogisticsController::class)->parameter('logistics', 'id');
});
Route::get('mailable', function () {
    $order = \App\Models\Order::query()->find(12);

    return new \App\Mail\ShippedMailToCustomer($order);
});
